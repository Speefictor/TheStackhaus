...
export async function createCheckoutSession(items: Item[]) {
  ...
  const requestHost = headersList.get('host') || 'localhost:9002';
  
  // Prioritize NEXT_PUBLIC_HOST, then fall back to request headers.
  const host = process.env.NEXT_PUBLIC_HOST || requestHost;
